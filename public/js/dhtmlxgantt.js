...
var dict = {
	...
	"confirm_deleting": {
		en: "Task will be deleted permanently, are you sure?",
		es: "La tarea sera eliminada permanente, ¿Estas seguro?"
	},
	"section_description": {
		en: "Description",
		es: "Descripcion"
...
"undefined" == typeof window.dhx4 && (window.dhx4 = {
		...
		selectTextRange: function(t, e, n) {
			...
			if (e = Math.max(Math.min(e, a), 0), n = Math.min(n, a),
...
function() {
	function t(t, e) {
		var a = t.callback;
		n(!1), t.box.parentNode.removeChild(t.box), c = t.box = null, a && a(e)
	}
	...
	function s(e, n, i) {
		...
		var r = "";
		if (e.width && (s.style.width = e.width), e.height && (s.style.height = e.height), e.title && (r += '<div class="dhtmlx_popup_title">' + e.title + "</div>"), r += '<div class="dhtmlx_popup_text"><span>' + (e.content ? "" : e.text) + '</span></div><div  class="dhtmlx_popup_controls">', n && (r += a(e.ok || "OK", !0)), i && (r += a(e.cancel || "Cancel", !1)), e.buttons)
	...
	var g = dhtmlx.message = function(t, e, n, a) {
	...
	if (t.getBoundingClientRect) {
		var a = t.getBoundingClientRect(),
			...
			d = s.clientTop || i.clientTop || 0,
			l = s.clientLeft || i.clientLeft || 0;
		return e = a.top + r - d, n = a.left + o - l, {
			y: Math.round(e),
			x: Math.round(n),
			width: t.offsetWidth,
	...
	var a = document.createElement("DIV");
	a.id = "gantt_" + dhtmlx.uid(), a.style.width = "100%", a.style.height = "100%", a.cmp = "grid", document.body.appendChild(a), this.attachObject(a.id), n = n || window.gantt, n.init(a.id, t, e), a.firstChild.style.border = "none";
	...
	for (var e = this.getGridColumns(), n = [], a = 0; a < e.length; a++) {
		...
		dhtmlx.defined(o.align) && (c += "text-align:" + o.align + ";"), i = "<div class='" + d + "' style='" + c + "'>" + l + s + "</div>",
		n.push(i)
	}
	...
	t.attachEvent("onBeforeDragStart", dhtmlx.bind(function(e, n) {
		...
		if (this.isTaskExists(d) || (d = t.lastTaskOfLevel(gantt.config.order_branch_free ? l.$level : 0), d == t.config.id && (d = null)),
			this.isTaskExists(d)) {
			...
			if (_.y + o.offsetHeight / 2 < r) {
				...
				if (g) {
					if (g.id == l.id) return;
					h = g
				} else if (g = this._pull[this._order[c]], a(g, l) && g.id != l.id) return void this.moveTask(l.id, -1, this.getParent(g.id))
			}
			for (var c = this.getGlobalTaskIndex(h.id), u = this._pull[this._order[c - 1]], f = 1;
				(!u || u.id == h.id) && c - f >= 0;) u = this._pull[this._order[c - f]], f++;
			if (l.id == h.id) return;
			a(h, l) && l.id != h.id ? this.moveTask(l.id, 0, 0, h.id) : h.$level != l.$level - 1 || gantt.getChildren(h.id).length ? u && a(u, l) && l.id != u.id && this.moveTask(l.id, -1, this.getParent(u.id)) : this.moveTask(l.id, 0, h.id);
		}
		return !0
	...
	adjustSize: function(t, e, n, a) {
		...
		for (var i = a - n + 1, s = this.getSum(e, n, a), r = 0, o = n; a >= o; o++) {
			var d = Math.floor(t * (s ? e[o] / s : 1 / i));
			s -= e[o], t -= d, i--, e[o] += d, r += d
		}
		e[e.length - 1] += t
	},
	...
	prepareConfigs: function(t, e, n, a) {
		for (var i = this.splitSize(a, t.length), s = n, r = [], o = t.length - 1; o >= 0; o--) {
			...
			d && this.processIgnores(l), this.initColSizes(l, e, s, i[o]), this.limitVisibleRange(l), d && (s = l.full_width), r.unshift(l)
		}
	...
	iterateScales: function(t, e, n, a, i) {
		for (var s = e.trace_x, r = t.trace_x, o = n || 0, d = a || r.length - 1, l = 0, _ = 1; _ < s.length; _++)
			for (var h = o; d >= h; h++) + r[h] != +s[_] || (i && i.apply(this, [l, _, o, h]), o = h, l = _)
	},
	...
	limitVisibleRange: function(t) {
		...
		if (i) {
			for (var l = this.getSum(t.width), _ = 0, h = 0; h < t.width.length; h++) {
				var c = Math.floor(i * (t.width[h] / l));
				t.width[h] += c,
				_ += c
			}
	...
	on_mouse_up: function(t) {
		...
		if (e.mode && e.id) {
			...
			if (gantt.config.work_time && gantt.config.correct_work_time && this._fix_working_times(n, e), this._fix_dnd_scale_time(n, e), gantt._init_task_timing(n), this._fireEvent("before_finish", e.mode, [e.id, e.mode, dhtmlx.copy(e.obj), t])) {
				...
				this._fireEvent("after_finish", e.mode, [a, e.mode, t]), this.clear_drag_state(), gantt.updateTask(n.id)
	...
	for (var o = 0; o < a.length; o++) {
		...
		var d = n.render_line(a[o], a[o + 1]);
		t.color && (d.firstChild.style.backgroundColor = t.color), i.appendChild(d)
	}
	...
	return n.style.cssText = ["top:" + i + "px", "left:" + s + "px"].join(";"), n.className = d, n
}, gantt._drawer = {
	current_pos: null,
	dirs: {
		left: "left",
	...
	get_points: function(t) {
		...
		if (t.type == gantt.config.links.start_to_start) this.point_to(s.left, r), o ? (this.point_to(s.down, a), this.point_to(s.right, i)) : (this.point_to(s.right, i), this.point_to(s.down, a)), this.point_to(s.right, r);
			...
			if (o = e.e_x > e.x + 2 * r, this.point_to(s.right, r), o) i -= r, this.point_to(s.down, a), this.point_to(s.right, i);
				...
				i -= 2 * r;
				var d = a > 0 ? 1 : -1;
				this.point_to(s.down, d * (n.row_height / 2)), this.point_to(s.right, i), this.point_to(s.down, d * (Math.abs(a) - n.row_height / 2)), this.point_to(s.right, r)
			...
			if (o = e.e_x > e.x - 2 * r, this.point_to(s.left, r), o) {
				...
				this.point_to(s.right, i), this.point_to(s.down, d * (Math.abs(a) - n.row_height / 2)), this.point_to(s.left, r)
	...
	function s(t, e, n, i) {
		...
		var h = Math.sqrt(Math.pow(n - t, 2) + Math.pow(i - e, 2));
		if (h = Math.max(0, h - 3)) {
			s.className = _.join(" ");
			var c = (i - e) / (n - t),
				g = Math.atan(c);
			2 == o(t, n, e, i) ? g += Math.PI : 3 == o(t, n, e, i) && (g -= Math.PI);
			var u = Math.sin(g),
				f = Math.cos(g),
				p = Math.round(e),
				m = Math.round(t),
	...
	function d() {
		return _._direction || (_._direction = document.createElement("div"),
			gantt.$task_links.appendChild(_._direction)), _._direction
	}
	...
	_.attachEvent("onBeforeDragStart", dhtmlx.bind(function(e, n) {
		...
		if (gantt._locate_css(a, g)) {
			gantt._locate_css(a, h) && (gantt._link_source_task_start = !0);
		...
		return !1
	}, this)), _.attachEvent("onAfterDragStart", dhtmlx.bind(function(t, n) {
		e(_.config.marker)
	}, this)), _.attachEvent("onDragMove", dhtmlx.bind(function(a, i) {
		var r = _.config,
			o = _.getPosition(i);
		n(r.marker, o);
		var d = gantt._is_link_drop_area(i),
			l = gantt._link_target_task,
			h = gantt._link_landing,
			g = gantt._link_target_task_start,
			f = gantt.locate(i),
			p = !0;
		if (d && (p = !gantt._locate_css(i, c), d = !! f), gantt._link_target_task = f, gantt._link_landing = d, gantt._link_target_task_start = p, d) {
			var m = gantt.getTask(f),
				v = gantt._locate_css(i, u),
				k = 0;
		...
		var x = !(h == d && l == f && g == p);
		return x && (l && gantt.refreshTask(l, !1),
		...
		i(), t.from && gantt.refreshTask(t.from, !1), t.to && gantt.refreshTask(t.to, !1), l()
	...
	for (var s = 0; s < t.count; s++) {
		...
		var r = a.call(this, n),
			...
			_ = "";
		if (o) {
			d = "width:" + o + "px;", _ = "gantt_scale_cell" + (s == t.count - 1 ? " gantt_last_cell" : ""),
			l = i.call(this, n), l && (_ += " " + l);
	...
	if (this._is_chart_visible()) {
		...
		for (var r = this._get_resize_options(), o = r.x ? Math.max(this.config.autosize_min_width, 0) : this.$task.offsetWidth, d = i.prepareConfigs(s, this.config.min_column_width, o, a), l = this._tasks = d[d.length - 1], _ = [], h = this.templates.scale_row_class, c = 0; c < d.length; c++) {
		...
		t = _.join(""), e = l.full_width + this.$scroll_ver.offsetWidth + "px", n = l.full_width + "px", a += "px"
	}
	...
	return this.getState().selected_task == t.id && (_ += " gantt_selected"),
	a.className = _, a.style.height = gantt.config.row_height + "px", a.setAttribute(this.config.task_attribute, t.id), a
	...
	if (r.link_source_id == n && a.push("gantt_link_source"), r.link_target_id == n && a.push("gantt_link_target"),
		this.config.highlight_critical_path && this.isCriticalTask && this.isCriticalTask(s) && a.push("gantt_critical_task"), r.link_landing_area && r.link_target_id && r.link_source_id && r.link_target_id != r.link_source_id) {
		var o = r.link_source_id,
			d = r.link_source_start,
			l = r.link_target_start,
			_ = gantt.isLinkAllowed(o, n, d, l),
			h = "";
		h = _ ? l ? "link_start_allow" : "link_finish_allow" : l ? "link_start_deny" : "link_finish_deny", a.push(h)
	}
	return a.join(" ")
...
gantt.removeLinkLayer = function(t) {
	...
	this._load_type = a, dhx4.ajax.get(t, dhtmlx.bind(function(t) {
	...
	add_worktime: function(t, e, n, a) {
		...
		if (gantt.config.work_time) {
			for (; e > s;) {
				...
				this.is_working_unit(a > 0 ? i : r, n) && s++, i = r
			}
			return i
		}
	...
	get_closest_worktime: function(t) {
		...
		for ("past" == t.dir && (l = -1); !this.is_working_unit(n, e);) {
			...
			if (n = gantt.date.add(n, l, e), n = gantt._correct_dst_change(n, _, l, e), gantt.date[e + "_start"] && (n = gantt.date[e + "_start"](n)), d && (s ? a = n : i = n), s = !s, o++, o > r) return dhtmlx.assert(!1, "Invalid working time check"), !1
		}
...
gantt._is_flex_task = function(t) {
	...
	if (o) {
		for (var d = [], l = o.length - 1; l >= 0; l--) d[l] = this._pull[o[l]];
		d.sort(s);
		for (var l = 0; l < d.length; l++) o[l] = d[l].id, this.sort(t, e, o[l], !0)
	}
	i && this.render()
...
gantt.showLightbox = function(t) {
	...
	if (void 0 === t && (t = this.getLightboxType()), !this._lightbox || this.getLightboxType() != this._get_safe_type(t)) {
		...
		a += "</div>", e.innerHTML = a, gantt.config.drag_lightbox && (e.firstChild.onmousedown = gantt._ready_to_dnd,
	...
	for (var e = "", n = 0; n < t.length; n++) {
		...
		if (a) {
			...
			var i = t[n].hidden ? " style='display:none'" : "",
				s = "";
			t[n].button && (s = "<div class='gantt_custom_button' index='" + n + "'><div class='gantt_custom_button_" + t[n].button + "'></div><div>" + this.locale.labels["button_" + t[n].button] + "</div></div>"), this.config.wide_form && (e += "<div class='gantt_wrap_section' " + i + ">"), e += "<div id='" + t[n].id + "' class='gantt_cal_lsection'>" + s + this.locale.labels["section_" + t[n].name] + "</div>" + a.render.call(this, t[n]),
			e += "</div>"
		}
	}
	return e
	...
	gantt.lightbox_events = {}, gantt.lightbox_events.gantt_save_btn = function(t) {
		...
		if (n || (e = e.previousSibling, n = gantt._getClassName(e)), e && n && 0 === n.indexOf("gantt_btn_set") && (e = e.firstChild, n = gantt._getClassName(e)), e && n) {
			var a = dhtmlx.defined(gantt.lightbox_events[e.className]) ? gantt.lightbox_events[e.className] : gantt.lightbox_events["default"];
			return a(t, e)
		}
		return !1
	...
	for (var i = this._get_typed_lightbox_config(this.getLightboxType()), s = 0; s < i.length; s++) {
		...
		if (this.form_blocks[r.type]) {
			var o = document.getElementById(r.id).nextSibling,
				...
				l = gantt._resolve_default_mapping(i[s]),
			...
			switch (l) {
				...
				case "%H:%i":
					...
					for (t._time_values = []; s > f;) {
						var m = this.templates.time_picker(i);
						_ += "<option value='" + f + "'>" + m + "</option>", t._time_values.push(f), i.setTime(i.valueOf() + 60 * this._get_timepicker_step() * 1e3);
						var v = i.getDate() != p ? 1 : 0;
						f = 24 * v * 60 + 60 * i.getHours() + i.getMinutes()
					}
			}
			if (_) {
	...
	_fill_lightbox_select: function(t, e, n, a, i) {
		if (t[e + a[0]].value = n.getDate(), t[e + a[1]].value = n.getMonth(), t[e + a[2]].value = n.getFullYear(), dhtmlx.defined(a[3])) {
	...
	time: {
		...
		set_value: function(t, e, n, a) {
			...
			if (i.auto_end_date)
				for (var o = function() {
					_ = new Date(s[r[2]].value, s[r[1]].value, s[r[0]].value, 0, 0), h = gantt.calculateEndDate(_, 1), this.form_blocks._fill_lightbox_select(s, r.size, h, r, i)
			...
			this.form_blocks._fill_lightbox_select(s, 0, _, r, i), this.form_blocks._fill_lightbox_select(s, r.size, h, r, i)
		},
		get_value: function(t, e, n) {
			...
			var d = new Date(a[i[2]].value, a[i[1]].value, a[i[0]].value, s, r);
	...
	duration: {
		...
		set_value: function(t, e, n, a) {
			function i() {
				...
				h.innerHTML = gantt.templates.task_date(i)
			}
			...
			var r = a,
				...
				c = a._time_format_order;
			_[0].onclick = dhtmlx.bind(function() {
				s(-1 * this.config.duration_step)
			}, this), _[1].onclick = dhtmlx.bind(function() {
				s(1 * this.config.duration_step)
			}, this), o[0].onchange = i, o[1].onchange = i, o[2].onchange = i, o[3] && (o[3].onchange = i), l.onkeydown = dhtmlx.bind(function(t) {
				t = t || window.event;
			...
			}, this), l.onchange = dhtmlx.bind(function(t) {
				i()
			}, this);
			...
			gantt.form_blocks._fill_lightbox_select(o, 0, u, c, r), l.value = p, i()
		},
		_get_start_date: function(t, e) {
			...
			if (dhtmlx.defined(a[3])) {
				...
				isNaN(o) && r.hasAttribute("data-value") && (o = parseInt(r.getAttribute("data-value"), 10)), i = Math.floor(o / 60),
				s = o % 60
			}
	...
	setTransactionMode: function(t, e) {
		"object" == typeof t ? (this._tMode = t.mode || this._tMode, this._headers = this._headers || t.headers,
			this._payload = this._payload || t.payload) : (this._tMode = t, this._tSend = e), "REST" == this._tMode && (this._tSend = !1, this._endnm = !0)
	},
	escape: function(t) {
	...
	setUpdated: function(t, e, n) {
		if (!this._silent_mode) {
			...
			i && "updated" == n && (n = i), e ? (this.set_invalid(t, !1), this.updatedRows[a] = t, this.obj.setUserData(t, this.action_param, n), this._in_progress[t] && (this._in_progress[t] = "wait")) : this.is_invalid(t) || (this.updatedRows.splice(a, 1), this.obj.setUserData(t, this.action_param, "")),
			e || this._clearUpdateFlag(t), this.markRow(t, e, n), e && this.autoUpdate && this.sendData(t)
		}
	},
		...
		if (t) {
			...
			if ("GET" == this._tMode) dhx4.ajax.get(i + (-1 != i.indexOf("?") ? "&" : "?") + this.serialize(t, e), a);
				...
				if ("inserted" == s ? o = this.serialize(t, e) : "deleted" == s ? (d = "DELETE", r = r + ("/" == r.slice(-1) ? "" : "/") + e) : (d = "PUT", o = this.serialize(t, e), r = r + ("/" == r.slice(-1) ? "" : "/") + e), this._payload)
					for (var l in this._payload) r = r + dhtmlx.url(r) + this.escape(l) + "=" + this.escape(this._payload[l]);
				dhx4.ajax.query({
					url: r,
					method: d,
	...
	afterUpdate: function(t, e, n) {
		...
		var l = dhx4.ajax.xpath("//data/action", d);
	...
	"links" == a && (i["delete"] = gantt.deleteLink,
		i.isExist = gantt.isLinkExists), i.isExist.call(gantt, e) && i["delete"].call(gantt, e)
	...
	this._init_html_area(t), this._set_sizes(), this._clear_renderers(), this.resetLightbox(), this._update_flags(), this._init_touch_events(), this._init_templates(), this._init_grid(), this._init_tasks(), this._set_scroll_events(), dhtmlxEvent(this.$container, "click", this._on_click),
	dhtmlxEvent(this.$container, "dblclick", this._on_dblclick), dhtmlxEvent(this.$container, "mousemove", this._on_mousemove), dhtmlxEvent(this.$container, "contextmenu", this._on_contextmenu), this.callEvent("onGanttReady", []), this.render()
}, gantt._init_html_area = function(t) {
	"string" == typeof t ? this._obj = document.getElementById(t) : this._obj = t, dhtmlx.assert(this._obj, "Invalid html container: " + t);
	...
	1 * t == t && (this.$task.scrollLeft = t, this._save_scroll_state(t, void 0)), 1 * e == e && (this.$task_data.scrollTop = e,
		this.$grid_data.scrollTop = e, this._save_scroll_state(void 0, e));
	...
	if (!(this._x < 20 || this._y < 20)) {
		...
		this.$scroll_hor.style.display = o ? "block" : "none", this.$scroll_hor.style.height = (o ? a : 0) + "px", this.$scroll_hor.style.width = Math.max(this._x - (d ? a : 2), 0) + "px", this.$scroll_hor.firstChild.style.width = i + t + a + 2 + "px", this.$scroll_ver.style.display = d ? "block" : "none", this.$scroll_ver.style.width = (d ? a : 0) + "px", this.$scroll_ver.style.height = Math.max(this._y - (o ? a : 0) - this.config.scale_height, 0) + "px", this.$scroll_ver.style.top = this.config.scale_height + "px", this.$scroll_ver.firstChild.style.height = this.config.scale_height + s + "px";
	}
...
function() {
	dhtmlx.mixin(gantt.config, {
		...
		types: {
			task: dict["task"][language],
			project: dict["project"][language],
			milestone: dict["milestone"][language]
		},
		duration_unit: "day",
		work_time: !1,
		correct_work_time: !1,
	...
	a.grid = gantt, gantt.init(n.id, t, e),
	n.firstChild.style.border = "none", a.gridId = n.id, a.gridObj = n;
	var i = "_viewRestore";
	...
	date: {
		...
		month_short: [
			dict['jan_short'][language],
			dict['feb_short'][language],
			dict['mar_short'][language],
			dict['apr_short'][language],
			dict['may_short'][language],
			dict['jun_short'][language],
			dict['jul_short'][language],
			dict['aug_short'][language],
			dict['sep_short'][language],
			dict['oct_short'][language],
			dict['nov_short'][language],
			dict['dec_short'][language]
		],
		day_full: [
			dict['sun'][language],
			dict['mon'][language],
		...
		day_short: [
			dict['sun_short'][language],
			dict['mon_short'][language],
			dict['tue_short'][language],
			dict['wed_short'][language],
			dict['thu_short'][language],
			dict['fri_short'][language],
			dict['sat_short'][language]	
		]
	},
	labels: {
		new_task: dict['new_task'][language],
		icon_save: dict['icon_save'][language],
		icon_cancel: dict['icon_cancel'][language],
		icon_details: dict['icon_details'][language],
		icon_edit: dict['icon_edit'][language],
		icon_delete: dict['icon_delete'][language],
		confirm_closing: "",
		confirm_deleting: dict['confirm_deleting'][language],
		section_description: dict['section_description'][language],
		section_time: dict['section_time'][language],
		section_type: dict['section_type'][language],
		column_text: dict['column_text'][language],
		column_start_date: dict['column_start_date'][language],
		column_duration: dict['column_duration'][language],
		column_add: "",
		link: dict['link'][language],
		confirm_link_deleting: dict['confirm_link_deleting'][language],
		link_start: dict['link_start'][language],
		link_end: dict['link_end'][language],
		type_task: dict['type_task'][language],
		type_project: dict['type_project'][language],
		type_milestone: dict['type_milestone'][language],
		minutes: dict['minutes'][language],
		hours: dict['hours'][language],
		days: dict['days'][language],
		weeks: dict['weeks'][language],
		months: dict['months'][language],
		years: dict['years'][language]
	}
}, gantt.skins.skyblue = {
	config: {
		grid_width: 350,
		...
		if (n && gantt.isTaskVisible(t))
			for (var a in e)
				if (n = e[a][t], n && n.getAttribute("task_id") && n.getAttribute("task_id") == t) {
					var i = n.cloneNode(!0);
					return h = n, e[a][t] = i, n.style.display = "none", i.className += " gantt_drag_move ", n.parentNode.appendChild(i), i
				}
	}
	var s, r = 0,
		o = !1,
	...
	this._gantt_touch_event_ready || (this._gantt_touch_event_ready = 1,
			...
			if (!n(t) && o) {
				...
				if (i && l) {
					...
					if (!d && (Math.abs(h) > 5 || Math.abs(c) > 5) && (gantt._touch_scroll_active = d = !0, r = 0, s = gantt.getScrollState()), d) {
						...
						if (s.x != g.x && c > 2 * h || s.y != g.y && h > 2 * c) return a(t);
					}
				}
				return a(t)
			}
		...
		if (!n(t)) {
			...
			_ = setTimeout(function() {
				...
				!t || gantt._locate_css(l, "gantt_link_control") || gantt._locate_css(l, "gantt_grid_data") || (gantt._tasks_dnd.on_mouse_down(l), gantt._tasks_dnd.drag && gantt._tasks_dnd.drag.start_drag && (i(t),
					gantt._tasks_dnd._start_dnd(l), gantt._touch_drag = !0, gantt.refreshTask(t), gantt._touch_feedback())), _ = null
		...
		if (!n(t)) {
			...
			var a = e(t);
			gantt._tasks_dnd.on_mouse_up(a), h && (gantt.refreshTask(gantt.locate(h)), h.parentNode && (h.parentNode.removeChild(h), gantt._touch_feedback())), gantt._touch_scroll_active = o = d = !1, h = null
		}
	})
};
