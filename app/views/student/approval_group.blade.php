							...
							<tbody>
								@foreach($pending as $index => $item)
									...
									<tr id="{{$index+1}}">
										...
										<td style="width:6%">
											<button type="button" class="btn btn-primary btn-sm pull-right" onclick="approve('{{$item->_id}}', {{$index+1}})"> 
												{{Lang::get('register_group.approve')}}
											</button>
										</td>
										<td style="width:6%">
											<button type="button" class="btn btn-danger btn-sm pull-right" onclick="deny('{{$item->_id}}', {{$index+1}})"> 
